...
function calculate() {
  ...
  const breakEvenRate = rebillRevenueAt100 > 0
    ? (rebillRevenueNeeded / rebillRevenueAt100) * 100
    : 0;
  ...
  if (!hasRequiredInputs) {
    $("breakEvenText").textContent =
      "Enter your campaign(s) numbers to calculate the approval rate needed to break even.";
  } else if (rebillRevenueNeeded <= 0) {
    $("breakEvenText").textContent =
      "Your front-end revenue already covers ad spend and COGS before any first rebills are approved.";
  } else if (breakEvenRate > 100) {
    $("breakEvenText").textContent =
      "Even 100% first-rebill approval would not fully recover the current campaign(s) cost.";
  } else {
    $("breakEvenText").textContent =
      `You need about ${breakEvenRate.toFixed(1)}% of first rebills to successfully process for the campaign(s) to break even.`;
  }
  ...
  if (!hasRequiredInputs && frontRevenue <= 0) {
    ...
    if (projectedProfit < -nearBand) {
      card.classList.add("loss");
      status.textContent = "Loss";
    } else if (Math.abs(projectedProfit) <= nearBand) {
  ...
  $("beLabel").textContent = breakEvenRate > 100
    ? "BE >100%"
  ...
  $("mathLine").textContent =
    `${money(frontRevenue)} front-end + ${money(projectedRebillRevenue)} first rebill revenue − ${money(adSpend)} ad spend − ${money(totalCogs)} COGS = ${projectedProfit >= 0 ? "+" : ""}${money(projectedProfit)}.`;
...
function resetCalculator() {
  ["adSpend","frontRevenue","customers","rebillPrice","cogsPerOrder"].forEach((id) => {
    $(id).value = "";
  });
  ...
  document.querySelectorAll(".time-option").forEach((btn) => {
    btn.classList.toggle("active", btn.dataset.time === "30 days");
  });
